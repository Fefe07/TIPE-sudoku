...
int main() {
  ...
  */
  ...
  for (int nbGrille = 2; nbGrille <= results_size + 1; nbGrille++) {

    printf("Grille n %d\n", nbGrille);
    grid_diffs g = lecture_db_diffs(nbGrille, "grilles/Bases_de_donnees_evaluees.csv");
    //int** g2 = lecture(nbGrille, "grilles/top50000.txt");
    
    //printGrid(g.grid);
    difficulties[nbGrille - 2] = (g.D_TR-average_dtr)* (g.D_TR-average_dtr);
